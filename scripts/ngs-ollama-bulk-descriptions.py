...
# --- prompt templates ---
SHORT_PROMPT = """أنت تكتب وصفًا تسويقيًا قصيرًا لمنتج إلكتروني في متجر سعودي.
...
FULL_PROMPT = """أنت تكتب وصفًا تفصيليًا لمنتج إلكتروني في متجر سعودي.
...
def validate_ar(text: str, full: bool) -> Optional[str]:
    ...
    # Strip "Translation:" / "Sure," / "Here is" prefixes
    text = re.sub(r'^(translation\s*:|sure[,!]?\s*[a-z\s]*:|here\s+is[^:]*:)\s*', '', text, flags=re.I)
    # Strip qwen reasoning tags <think>...</think>
    ...
    if total > 0 and ar_chars / total < 0.55:
        return f'AR ratio {ar_chars/total:.2f} < 0.55'
    return None  # valid; mutates returned text via tuple
